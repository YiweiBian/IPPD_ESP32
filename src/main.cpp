...
void loop()
{
  ...
  if (inputComplete)
  {
    ...
    {
      int value = inputString.substring(2).toInt();
      driver.microsteps(value);
      // Recompute microstepsPerRev and stepIncrement if microstepping changes
      microstepsPerRev = FULL_STEPS_PER_REV * driver.microsteps();
      stepIncrement = (30 * microstepsPerRev) / 60;
    ...
    else if (inputString.startsWith("c "))
    {
      ...
      {
        Serial.println("⚠️ Invalid current value. Use 100-1200 mA.");
      }
    }
    else if (inputString.startsWith("f "))
    {
      int freq = inputString.substring(2).toInt();
      if (freq > 0)
      {
        setPumpFrequency(freq);
      }
      else
      {
        Serial.println("⚠️ Invalid frequency.");
      }
    }
    ...
    else if (inputString.equals("dir"))
    {
      reverseMotorDirection();
    }
    else if (inputString.equals("sread"))
    {
      read_sensor();
    }
    else
...
void handleNextionInput()
{
  while (nextion.available())
  {
    ...
    if (nextionBufIndex < NEXTION_BUF_SIZE)
    {
      nextionBuf[nextionBufIndex++] = b;
    }
    else
    {
      nextionBufIndex = 0;
      Serial.println("Nextion buffer overflow; resetting buffer.");
    }
    if (nextionBufIndex >= 3 &&
        nextionBuf[nextionBufIndex - 1] == 0xFF &&
        nextionBuf[nextionBufIndex - 2] == 0xFF &&
        nextionBuf[nextionBufIndex - 3] == 0xFF)
    {
      String cmd = "";
      for (size_t i = 0; i < nextionBufIndex - 3; i++)
      {
        cmd += (char)nextionBuf[i];
      }
      cmd.trim();
      cmd.toUpperCase();
      Serial.print("Parsed Nextion command: ");
      Serial.println(cmd);
      if (cmd == "RUN")
      {
        isRunning = true;
